            ...
            if self.sale_order_id:
                ...
                if self.sale_order_id.order_line:
                    for existing_line in self.sale_order_id.order_line:
                        ...
                        sequence+=1
                orders = self.env['sale.order'].sudo().search([('id','!=',self.sale_order_id.id),('id','in',self.sale_order_ids.ids)],order='id asc')
                for order in orders:
                    if order.order_line:
                ...
                if created_so:
                    ...
                    sequence = 10
                    orders = self.env['sale.order'].sudo().search([('id','in',self.sale_order_ids.ids)],order='id asc')
                    for order in orders:
                        if order.order_line:
                            for line in order.order_line:
                                merged_line = line.copy(default=order_line_vals)
